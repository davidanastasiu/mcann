...
#         super(DAN, self).__init__(opt)
    ...
    def test_single(self, test_point):
        ...
        gmm_prob30 = self.gmm.predict_proba(np.squeeze(np.array(x_test)[:,-1*self.gmm_l:,1:2]).reshape(1,-1))
...
#         total = math.sqrt(total)
        ...
        if total < min_RMSE:
            ...
            #save_model
            encoder_name = self.expr_dir + '/' + 'MCANN_encoder.pt'
            decoder_name = self.expr_dir + '/' + 'MCANN_decoder.pt'
    ...
    def generate_test_rmse_mape(self):
        ...
        avg_rmse = np.around(np.mean(np.array(val_rmse_list)),2)
    ...
    def compute_metrics(self, aa):
        ...
        metrics = metric_g("MC-ANN", np.array(all_DAN), np.array(all_GT))
        return metrics
    ...
    def train(self):
        ...
        for epoch in range(num_epochs):
            ...
            for i, batch in enumerate(self.dataloader):
                ...
                y_train = torch.squeeze(torch.from_numpy(np.array(y_train0, np.float32)).to(device),dim=2)
