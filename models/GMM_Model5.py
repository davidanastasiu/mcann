...
class EncoderLSTM(nn.Module):
    def __init__(self, opt):
        ...
        self.lstm0 = nn.LSTM(2, self.hidden_dim, self.layer_dim, dropout=0.1, bidirectional=False, batch_first=True)
        self.lstm1 = nn.LSTM(2, self.hidden_dim, self.layer_dim, dropout=0.1, bidirectional=False, batch_first=True)
        self.lstm2 = nn.LSTM(2, self.hidden_dim, self.layer_dim, dropout=0.1, bidirectional=False, batch_first=True)
...
class DecoderLSTM(nn.Module):
    ...
    def forward(self, x, encoder_h, encoder_c, ww): 
        ...
        out = torch.unsqueeze((torch.mul(out0,w0)+torch.mul(out1,w1)+torch.mul(out2,w2)),2)
