...
import random
import pandas as pd
from FlowView.models.MCANN.utils.utils2 import *
import os
import torch
import sklearn
...
#         super(DS, self).__init__()
    ...
    def read_dataset(self):
        ...
        if(self.is_prob_feature==1):
            ...
            prob_in_distribution3 = data_prob30[:, 0] * weights3[0] + data_prob30[:, 1] * weights3[1] + data_prob30[:, 2] * weights3[2]
            ...
            prob_like_outlier3 = np.array(recover_data, np.float32).reshape(len(gmm_input), 1)
    ...
    def train_dataloader(self):
        ...
        if(self.is_over_sampling == 1):
            ...
            while ii < self.opt.train_volume:
                ...
                a5 = self.iterval
                if (jj < self.opt.train_volume*(self.oversampling/100)) and (np.max(pre1) > self.thre2 or np.min(pre1) < self.thre1) and ( not np.isnan(self.sensor_data_norm1[i:i+self.lens]).any()) and (self.tag[i+self.train_days] <= a1 or a2 < self.tag[i+self.train_days] < 0):
                    if a3 > 0:
                        i = i + max_index -1
                        i = i - a3*a5
                    for kk in range(a3): #(int(self.predict_days/self.iterval)):
                        ...
                        if ( not np.isnan(self.sensor_data_norm1[i:i+self.lens]).any() and self.tag[i+self.train_days] != 2 and self.tag[i+self.train_days] != 3  and self.tag[i+self.train_days] != 4):
                            ...
                            label01 = np.array(self.diff_data[(i+self.train_days):(i+self.train_days+self.predict_days)])
...
#                     ii = ii + 1
        ...
        gmm_prob30 = self.gmm.predict_proba(np.squeeze(np.array(self.DATA)[:,-1*self.gmm_l:,1:2]))
